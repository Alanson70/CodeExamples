 ...
 if(OpenDialog1->Execute())
 {
  ...
  try
  {
   ...
   for(int i=0;i<MAX_FREQS;i++)
   {
    ...
    FS[i].AN=std::max(0.1,std::min(30.0,(double)IniFile->ReadFloat(Section,"Коэффициент",1)));
    FS[i].CM=std::max(0,std::min(3,IniFile->ReadInteger(Section,"Модуляция",1)));
    FS[i].FEC=std::max(0,std::min(2,IniFile->ReadInteger(Section,"Кодирование",0)));
...
void __fastcall TFreqForm::PrintItemClick(TObject *Sender)
{
 if(FreqPrintForm==NULL)
 {
  FreqPrintForm=new TFreqPrintForm(this);
  try
  {
   FreqPrintForm->ShowModal();
   delete FreqPrintForm;
   FreqPrintForm=NULL;
  }
  catch(...)
  {
